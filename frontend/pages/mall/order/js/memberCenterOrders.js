    ...
    fetch(config.url + "/user/getUserOrders", {
        ...
        .then(data => {
            if (data.message) {
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                if (type === 'display') {
                                    // 创建删除按钮元素
                                    const removeButton = '<a href="#" class="btn-link remove-icon"><i class="fa-solid fa-eraser"></i></a>';
                                    return removeButton;
                                }
                                return data; // 其他情况返回原始数据
                            }
                ...
                // 監聽取消訂單的點擊事件
                $('#OrdersTable tbody').on('click', 'a.remove-icon', function (e) {
                    e.preventDefault();
                    ...
                    if (ordPayStatus === 0 && paymentTransactionId) {
                        ...
                        Swal.fire({
                            title: '無法刪除',
                            text: '此訂單已付款過或完成付款，無法刪除，若要刪除請先取消付款或聯絡管理員',
                            icon: 'error',
                ...
                $('#OrdersTable tbody').on('click', 'a.delete-icon', function (e) {
                    ...
                    if (ordPayStatus === 0) {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                // 點擊確定 刪除!
                                paymentCancelOrder(requestData, selectedRow, paymentTransactionId, paymentMethod);
...
// 取消交易(API_3 paymentCancelOrder [取消交易])
function paymentCancelOrder(requestData, selectedRow, paymentTransactionId, paymentMethod) {
    ...
    fetch("https://cors-anywhere.herokuapp.com/https://test-api.fonpay.tw/api/payment/paymentCancelOrder", {
        ...
        .then(res => {
            console.log("2");
            if (res.response.errorCode == 0 && res.result.payment.status == "CANCEL") {
                deleteOrder(requestData, selectedRow);
            } else {
                console.log("1");
                Swal.fire({
